...
[.lang-python]
NOTE: In Python the producer is an abstract class (`ABC`) whose `@abstractmethod` methods have `...` bodies; the parameter binding annotations are applied with `typing.Annotated`.
...
At compile time Micronaut will produce an implementation of the interface. You can retrieve an instance of `TextProducer` either by looking up the bean from the link:{apimicronaut}context/ApplicationContext.html[ApplicationContext] or with dependency injection:
...
The following example will listen for messages published by the `TextProducer` in the previous section:
